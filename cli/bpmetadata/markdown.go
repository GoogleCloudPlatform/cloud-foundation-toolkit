...
func getMdContent(path []byte, headLevel int, headOrder int, headTitle string, getContent bool) *mdContent {
	mdDocument := markdown.Parse(path, nil)
	orderCtr := 0
	if mdDocument != nil {
		...
		for _, section := range mdSections {
			...
			// if currLeaf != nil {
			// 	string(currLeaf.Literal)
			// }
			// .Literal
			//currLiteralVal := string(ast.GetFirstChild(section).AsLeaf().Literal)
