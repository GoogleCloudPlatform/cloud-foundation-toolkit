...
locals {
  ...
  owners        = { for value in var.repos_map : value.name => "${join(" ", formatlist("@%s", setunion(lookup(value, "admins", []), lookup(value, "maintainers", []))))} " if length(setunion(lookup(value, "admins", []), lookup(value, "maintainers", []))) > 0 }
